...
export class GroupRequestsService {
    ...
    private loadRequests(): void {
        ...
        if (storedRequests) {
            ...
            const sortedRequests = requests.sort((a: GroupInterestRequest, b: GroupInterestRequest) =>
                b.requestedAt.getTime() - a.requestedAt.getTime()
            );

            this.requestsSubject.next(sortedRequests);
            this.updateStats(sortedRequests);
        } else {
    ...
    private initializeDefaultRequests(): void {
        ...
        // Sort by requestedAt descending (newest first)
        const sortedRequests = defaultRequests.sort((a: GroupInterestRequest, b: GroupInterestRequest) =>
            b.requestedAt.getTime() - a.requestedAt.getTime()
        );

        localStorage.setItem('groupInterestRequests', JSON.stringify(sortedRequests));
        this.requestsSubject.next(sortedRequests);
        this.updateStats(sortedRequests);
    }
    ...
    private saveRequests(requests: GroupInterestRequest[]): void {
        // Sort by requestedAt descending (newest first) before saving
        const sortedRequests = requests.sort((a: GroupInterestRequest, b: GroupInterestRequest) =>
